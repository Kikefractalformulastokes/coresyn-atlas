...
## Unreleased — PR #8 update: Kroon/Matmerize evidence upgrade + HASH_MISMATCH conflict (same branch)

- Following founder review of `CODEX_ATLAS_HANDOFF_REVIEW.md` and an independent Gmail check this session ran on its own initiative (never trusted a third party's description of that search), two Project/Person Registry entries had real, new, independently-verified evidence: `person-kroon` (now `VERIFIED`, identified as Prof. Juan A. Valiente Kroon, QMUL, real ongoing external-review correspondence — `roles` now includes `REVIEWER`, so this entry now surfaces in the derived `reviewer_registry.json` for the first time, 0 → 1) and `proj-ext-matmerize` (now `WAITING_FOR_NEW_EVIDENCE`, identified as a real materials-licensing company, real correspondence via Prof. Rampi Ramprasad/Georgia Tech and Hansa Makhija/Matmerize).
- Per explicit founder instruction, the prior status is never overwritten silently: both entries gained a new `evidence_history` field (append-only, new field added to `schemas/person-registry.schema.json` and `schemas/project-registry.schema.json`) recording the prior `UNVERIFIED_IN_THIS_SCOPE`/`NOT_VERIFIED_IN_THIS_SCOPE` state and exactly what search did/didn't find it.
- `proj-ext-crm-cfd` and `proj-ext-nashgate` re-checked via the same Gmail search — both remain unverified (CRM CFD: 3 matches, all false positives; NashGate: 0 matches) — status unchanged, one more independently-checked negative added to `verification_method`.
- New `HASH_MISMATCH` conflict logged (`conf-hash-mismatch-coresyn-master-registry-v2-proposed`): a separate Drive handoff artifact (`CORESYN_MASTER_REGISTRY.v2.proposed.json`, from a node self-identifying as `CODEX-NODE-03`) has a claimed SHA-256 that does not match the file this session independently downloaded and hashed twice. `canonical_impact: UNCHANGED` — that artifact isn't part of this repository's canonical files; nothing here was altered because of it, per explicit founder instruction not to touch anything based on unresolved external evidence.
- `state: UNDER_REVIEW` on all 6 conflicts now in `conflict_registry.json` — none auto-resolved.
